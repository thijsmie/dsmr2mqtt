...
from dsmr2mqtt import dsmr50 as dsmr
from dsmr2mqtt.log import logger
            ...
            if int(dsmr.definition[index][dsmr.HA_INCLUDED]):
                ...
                if len(tag_matches) == len(description_matches) == re.compile(regex).groups:
                    while i < re.compile(regex).groups:
                        ...
                        d["state_topic"] = (
                            cfg.MQTT_TOPIC_PREFIX + "/" + dsmr.definition[index][dsmr.MQTT_TOPIC]
                        )
                        ...
                        #              d["state_class"] = "measurement"
                            ...
                            d["unit_of_measurement"] == "m3"
                            or d["unit_of_measurement"] == "m\u00b3"
                        ):
                            d["device_class"] = "gas"
                            d["state_class"] = "total"
                            ...
                            d["value_template"] = (
                                "{{value_json." + tag_matches[i] + "|float/1000|round(0)" + "}}"
                            )
                        else:
                            logger.debug(
                                "unknown_unit_of_measurement", unit=d["unit_of_measurement"]
                            )
                        ...
                        logger.debug(
                            "sensor_config_created", unique_id=d["unique_id"], name=d["name"]
                        )

                        self.__listofjsondicts.append(d)
                else:
                    logger.warning(
                        "dsmr_definition_mismatch", tag=tag, regex=regex, description=description
                    )
            ...
            while not self.__stopper.is_set():
                ...
                if t_elapsed > self.__interval:
                    for _dict in self.__listofjsondicts:
                        ...
                        self.__mqtt.do_publish(
                            topic, json.dumps(_dict, separators=(",", ":")), retain=True
                        )
